...
A function that gives the cut points to separate data in parts of the same size. It's intended to be used along with the "cut" function.

```{r}
low_cut_break <- function(dataframe, variable_chr, cut){
  
  low_cut <- min(dataframe[,variable_chr], na.rm=TRUE) + ((max(dataframe[,variable_chr], na.rm=TRUE)-min(dataframe[,variable_chr], na.rm=TRUE))/cut)
  
  return(low_cut)
}

high_cut_break <- function(dataframe, variable_chr, cut){
  
  high_cut <- max(dataframe[,variable_chr], na.rm=TRUE) - ((max(dataframe[,variable_chr], na.rm=TRUE)-min(dataframe[,variable_chr], na.rm=TRUE))/cut)
